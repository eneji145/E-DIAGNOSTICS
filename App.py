...
def predict():
    if 'user_id' in session:
        ...
        # Making predictions
        RFC_predict = RFC1.predict(df)
        DTC_predict = DTC1.predict(df)
        SVC_predict = SVC1.predict(df)
        NB_predict = NB1.predict(df)
        
        all_predictions = [RFC_predict, DTC_predict, SVC_predict, NB_predict]
